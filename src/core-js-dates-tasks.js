...
function getNextFriday(date) {
  ...
  let daysRemain = Math.abs(data.getDay() - 12) % 7;
  if (daysRemain === 0) daysRemain = 7;
  const newSeconds = Date.parse(data) + 1000 * 60 * 60 * 24 * daysRemain;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const secs = dateTwo - dateOne;
  return secs / (1000 * 60 * 60 * 24) + 1;
}
...
function getWeekNumberByDate(date) {
  const januaryFirst = new Date(date.getFullYear(), 0);
  const daysToNextMonday =
    januaryFirst.getDay() === 1 ? 0 : (8 - januaryFirst.getDay()) % 7;
  let nextMonday = new Date(date.getFullYear(), 0, daysToNextMonday);
  let week = daysToNextMonday === 0 ? 0 : 1;

  while (nextMonday.getTime() < date.getTime()) {
    week += 1;
    nextMonday = new Date(nextMonday.getTime() + 1000 * 3600 * 24 * 7);
  }
  return week;
}
...
function getNextFridayThe13th(date) {
  ...
  while (nextFriday.getDate() !== 13) {
    nextFriday = new Date(nextFriday.getTime() + 1000 * 3600 * 24 * 7);
  }
...
function getQuarter(date) {
  const januaryFirst = new Date(date.getFullYear(), 0);
  const DAYS_OF_YEAR = date.getFullYear() % 4 === 0 ? 366 : 365;
  const QUARTER_DAYS = Math.trunc(DAYS_OF_YEAR / 4);

  return Math.ceil(
    (date.getTime() - januaryFirst.getTime()) /
      (QUARTER_DAYS * 1000 * 3600 * 24)
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (nextDay.getTime() <= end.getTime()) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (nextDay.getTime() > end.getTime()) break;
      schedule.push(dateToString(nextDay));
      nextDay = new Date(nextDay.getTime() + 1000 * 3600 * 24);
    }
    nextDay = new Date(nextDay.getTime() + 1000 * 3600 * 24 * countOffDays);
  }
